...
stats_<version>.csv, plus a per-barcode breakdown for the multiplex
barcode-kit case, and a summary_all_versions.csv across every version run
...
STATS_COLUMNS = [
    "dorado_version", "analyte", "library", "test_name", "model", "mods",
    "status", "error_message", "wall_time_sec",
    "num_reads", "num_reads_passed", "num_bases", "num_bases_passed",
    "n50", "read_len_mean", "read_len_median", "read_len_mode", "mean_qscore",
    "polya_median", "polya_mean",
    "commands_executed",
]

BARCODE_KIT_TEST_NAME = "dna_multiplex_barcode_kit"
...
def _case_bam_paths(case: dict) -> list[Path]:
    ...
    if case["test_name"] == BARCODE_KIT_TEST_NAME:
        groups = stats.discover_barcode_bams(output_dir / "demux")
        return [bam for bams in groups.values() for bam in bams]
    return dorado_commands.find_calls_bams(output_dir)
...
def build_case_row(dorado_path: str, dorado_version: str, case: dict) -> dict:
    row = {
        ...
        "commands_executed": " ; ".join(case.get("commands_executed") or []),
    }
    ...
    if not bam_paths:
        row["status"] = "failed"
        row["error_message"] = "No output BAM files found for stats"
        return row
    ...
    try:
        ...
        row["status"] = "failed"
        row["error_message"] = f"Stats computation failed: {exc}"
    return row
...
def aggregate_version(version_dir: Path) -> Path:
    ...
    barcode_case = next(
        (c for c in manifest["cases"] if c["test_name"] == BARCODE_KIT_TEST_NAME and c["status"] == "success"),
        None,
    )
    if barcode_case is not None:
        per_barcode_rows = build_per_barcode_rows(dorado_path, dorado_version, barcode_case)
        if per_barcode_rows:
...
def main(argv: list[str] | None = None) -> int:
    ...
    for version_dir in version_dirs:
        if not (version_dir / "manifest.json").is_file():
            print(f"Skipping {version_dir}: no manifest.json found")
            continue
