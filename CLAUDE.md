...
- **Model complex + mods:** append comma-separated mod codes to the model, e.g. `dorado basecaller hac,5mCG_5hmCG,6mA <data>`. Equivalent alternative: `--modified-bases <space separated codes>`. **This only auto-resolves against a floating speed alias** (`hac`/`sup`/`fast`) — verified against v2.1.0: appending bare mod codes to an already-pinned `model@version` (e.g. `dna_..._hac@v6.0.0,5mCG_5hmCG`) fails with `'<code>' is not a recognised model name`, even for a code that's valid and non-conflicting. For a pinned model, resolve each mod code to its fully-qualified name yourself (`<pinned_model>_<code>@<version>`, found via `dorado download --list`) and pass the comma-joined result through `--modified-bases-models` instead — see `run_compare_models.py`'s `_resolve_mod_models`. `_resolve_mod_models` returns `(resolved, missing)` rather than an all-or-nothing result: a pinned model is never dropped from the comparison just because one requested mod code doesn't exist for it — it basecalls with whichever mods did resolve (or with none at all, same as omitting `--mods`, if none did), so every requested version still gets a row.
